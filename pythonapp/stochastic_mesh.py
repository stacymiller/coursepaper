...
def diffusion_mesh_generator(initial_price, drift, volatility, M, T=1):
    dt = T / M
    ...
    return lambda t: initial_price * np.exp((drift - sigma_sq / 2) * t * dt + normal(zeros, volatility))
...
def lognormal_weights(initial_price, drift, volatility, M, T=1):
    dt = T / M
